...
COOLDOWN_CONFIG = {
    'booster': 45 * 60,  # 45 minutos en segundos
    'user': 60 * 60,     # 1 hora en segundos
    'tag': 60 * 60,      # 1 hora en segundos
    'default': 60 * 60   # 1 hora por defecto
}
...
for file_path in [ACCOUNTS_FILE, LOGS_FILE, KEYS_FILE, INVITES_FILE, COOLDOWNS_FILE]:
...
def load_cooldowns():
    """Carga los datos de cooldowns desde el archivo JSON."""
    global cooldowns_data
    try:
...
def save_cooldowns():
    """Guarda los datos de cooldowns en el archivo JSON."""
    try:
        with open(COOLDOWNS_FILE, 'w', encoding='utf-8') as f:
            json.dump(cooldowns_data, f, indent=4)
    except Exception as e:
        print(f"Error guardando cooldowns: {e}")

def clean_expired_cooldowns():
    """Limpia los cooldowns expirados del sistema."""
    now = datetime.now().timestamp()
    expired_users = []
    
    for user_id, cooldown_end in cooldowns_data.items():
        if cooldown_end and now > cooldown_end:
            expired_users.append(user_id)
    
    for user_id in expired_users:
        del cooldowns_data[user_id]
    
    if expired_users:
        save_cooldowns()

def get_user_cooldown(user):
    """Determina el cooldown apropiado para un usuario basado en sus roles."""
    # Buscar roles en orden de prioridad (cooldown más corto primero)
    role_priorities = ['booster', 'user', 'tag']
    
    user_roles = [role.name.lower() for role in user.roles]
    
    for role_name in role_priorities:
        if role_name in user_roles:
            return COOLDOWN_CONFIG[role_name]
    
    return COOLDOWN_CONFIG['default']

def can_use_cuenta_command(user):
    """Verifica si un usuario puede usar el comando /cuenta."""
    user_id = str(user.id)
    now = datetime.now().timestamp()
    
    clean_expired_cooldowns()
    
    if user_id in cooldowns_data:
        cooldown_end = cooldowns_data[user_id]
        if now < cooldown_end:
...
def set_cooldown(user):
    """Establece el cooldown para un usuario después de usar /cuenta."""
    user_id = str(user.id)
    cooldown_duration = get_user_cooldown(user)
    cooldown_end = datetime.now().timestamp() + cooldown_duration
    
    cooldowns_data[user_id] = cooldown_end
    save_cooldowns()
    
    return cooldown_end

def format_cooldown_time(cooldown_end):
    """Formatea el tiempo restante de cooldown para mostrarlo al usuario."""
    now = datetime.now().timestamp()
...
# --- Sistema de Tracking de Invitaciones MEJORADO ---

def analyze_user_invites(invites_data, user_id, guild):
    """Analiza las invitaciones de un usuario y las categoriza - VERSIÓN CORREGIDA."""
    ...
    for invited_user_id, invite_data in user_invites.items():
        ...
        # Verificar si se fue (left) - pero no es fake
        if invite_data.get('left_at'):
            left_invites.append({
                'user_id': invited_user_id,
                'username': invite_data['username'],
        ...
        if invited_user:
            ...
            account_age_days = 0
            if 'account_created' in invite_data:
                try:
                    account_created = datetime.fromisoformat(invite_data['account_created'])
                    account_age_days = (join_time - account_created).days
                except:
                    # Si hay error en el formato, usar join_time como referencia
                    account_age_days = 0
            
            if account_age_days < 7:  # Cuenta creada hace menos de 7 días
                alt_invites.append({
                    'user_id': invited_user_id,
                    'username': invite_data['username'],
            ...
            # Usuario no está en el servidor pero no tiene left_at registrado
            left_invites.append({
                'user_id': invited_user_id,
                'username': invite_data['username'],
                'joined_at': invite_data['joined_at'],
                'left_at': 'Desconocido | Unknown'
            })
...
# *** Tarea para limpiar cooldowns expirados periódicamente ***
@tasks.loop(minutes=30)
async def clean_cooldowns_task():
    """Limpia cooldowns expirados cada 30 minutos."""
    clean_expired_cooldowns()
...
# NUEVO EVENTO: Tracking de invitaciones MEJORADO
@bot.event
async def on_member_join(member):
    """Registra cuando un usuario se une al servidor - VERSIÓN MEJORADA"""
    try:
        invites_data = load_invites_data()
        ...
        # Guardar los uses actuales primero
        current_invites = {}
        for invite in invites:
            current_invites[str(invite.id)] = invite.uses
        
        # Buscar qué invitación fue usada comparando con los datos guardados
        used_invite = None
        used_inviter = None
        
        for invite_id, current_uses in current_invites.items():
            previous_uses = invites_data['invites'].get(invite_id, {}).get('uses', 0)
            if current_uses > previous_uses:
                # Esta es la invitación usada
                invite_obj = next((inv for inv in invites if str(inv.id) == invite_id), None)
                if invite_obj:
                    used_invite = invite_obj
                    used_inviter = invite_obj.inviter
                    break
        
        # Si no encontramos por comparación, usar la primera invitación con uses > 0
        if not used_invite:
            for invite in invites:
                if invite.uses > 0:
                    used_invite = invite
        ...
        if used_invite and used_inviter:
            inviter_id = str(used_inviter.id)
            ...
            invites_data['user_invites'][inviter_id][invited_id] = {
                ...
                'inviter_name': used_inviter.name
            }
            ...
            invites_data['invites'][str(used_invite.id)] = {
            ...
            print(f"📥 Usuario {member.name} se unió mediante invitación de {used_inviter.name} (Código: {used_invite.code})")
        else:
            print(f"📥 Usuario {member.name} se unió pero no se pudo determinar la invitación")
...
async def cuenta_command(interaction: discord.Interaction):
    """Obtener una cuenta del inventario - CON SISTEMA DE COOLDOWN"""
    ...
    # Verificar cooldown
    can_use, cooldown_end = can_use_cuenta_command(interaction.user)
    
    if not can_use:
        remaining_time = format_cooldown_time(cooldown_end)
        
        embed = discord.Embed(
            title="⏰ Cooldown Activo | Cooldown Active",
            description=(
                f"Debes esperar antes de usar este comando nuevamente. | You must wait before using this command again.\n\n"
                f"**Tiempo restante | Time remaining:** {remaining_time}\n"
                f"**Podrás usar nuevamente | You can use again:** <t:{int(cooldown_end)}:R>"
            ),
            color=discord.Color.orange()
        )
        
        # Información sobre cooldowns por rol
        cooldown_info = (
            "**Cooldowns por rol | Cooldowns by role:**\n"
            "• 🚀 **Boosters:** 45 minutos | 45 minutes\n"
            "• 👤 **Users:** 1 hora | 1 hour\n"  
            "• 🏷️ **Tags:** 1 hora | 1 hour\n"
            "• ⭐ **Default:** 1 hora | 1 hour"
        )
        embed.add_field(name="ℹ️ Información | Information", value=cooldown_info, inline=False)
        embed.set_footer(text="HMFB X")
    ...
    # ESTABLECER COOLDOWN después de obtener la cuenta exitosamente
    cooldown_end = set_cooldown(interaction.user)
    cooldown_duration = get_user_cooldown(interaction.user)
    
    try:
        # EMBED BILINGÜE PARA CUENTA OBTENIDA CON ADVERTENCIA
        embed = discord.Embed(
        ...
        # *** ADVERTENCIA AÑADIDA AQUÍ ***
        embed.add_field(
            name='⚠️ Advertencia | Warning',
            value='Algunas cuentas pueden dejar de funcionar por Cambios de contraseñas o bloqueos de Microsoft | Some accounts may stop working due to Password changes or Microsoft blocks',
            inline=False
        )
        ...
        embed.add_field(
            name='⏰ Cooldown | Tiempo de espera',
            value=f'Podrás solicitar otra cuenta en {cooldown_minutes} minutos | You can request another account in {cooldown_minutes} minutes',
            inline=False
        )
...
# NUEVO COMANDO: Sistema de invitaciones - COMPLETAMENTE CORREGIDO
...
async def invites_command(interaction: discord.Interaction, usuario: discord.Member = None):
    """Analiza las invitaciones de un usuario - VERSIÓN COMPLETAMENTE CORREGIDA"""
    ...
    if analysis['valid']:
        sample_valid = analysis['valid'][:3]  # Mostrar máximo 3 ejemplos
        valid_examples = "\n".join([f"• {invite['username']}" for invite in sample_valid])
        if len(analysis['valid']) > 3:
            valid_examples += f"\n• ... y {len(analysis['valid']) - 3} más"
        embed.add_field(
            name="👥 Ejemplos Válidos | Valid Examples",
...
# NUEVO COMANDO: Resetear invitaciones - MEJORADO CON PARÁMETRO OPCIONAL
@bot.tree.command(name="reset-invites", description="Resetea las estadísticas de invitaciones (Admin)")
@app_commands.checks.has_permissions(administrator=True)
@app_commands.describe(
    usuario="Usuario específico a resetear (opcional, dejar vacío para resetear todo)"
)
async def reset_invites_command(interaction: discord.Interaction, usuario: discord.Member = None):
    """Resetea las estadísticas de invitaciones - VERSIÓN MEJORADA CON USUARIO OPCIONAL"""
    
    if usuario:
        # Resetear solo un usuario específico
        confirm_embed = discord.Embed(
            title="⚠️ Confirmar Reset de Invitaciones | Confirm Invites Reset",
            description=(
                f"**¿Estás seguro de que quieres resetear las estadísticas de invitaciones de {usuario.mention}?**\n"
                f"**Are you sure you want to reset the invite statistics for {usuario.mention}?**\n\n"
                f"🗑️ **Esto eliminará:** | **This will delete:**\n"
                f"• Todas las invitaciones registradas de {usuario.display_name} | All registered invites from {usuario.display_name}\n"
                f"• Historial de joins/leaves asociado | Associated join/leave history\n"
                f"• Estadísticas específicas de este usuario | This user's specific statistics\n\n"
    ...
    class ResetConfirmView(discord.ui.View):
        ...
        async def confirm_reset(self, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            if self.target_user:
                ...
                # Eliminar del user_invites
                if user_id_str in invites_data['user_invites']:
                    del invites_data['user_invites'][user_id_str]
                
                # Eliminar join_events donde este usuario fue el invitador
                join_events_to_remove = []
                for invited_id, event_data in invites_data['join_events'].items():
                    if event_data.get('inviter_id') == user_id_str:
                        join_events_to_remove.append(invited_id)
                
                for invited_id in join_events_to_remove:
                    del invites_data['join_events'][invited_id]
                ...
                success_embed = discord.Embed(
                    ...
                    description=(
                        f"**Las estadísticas de invitaciones de {self.target_user.mention} han sido reseteadas correctamente.**\n"
                        f"**The invite statistics for {self.target_user.mention} have been successfully reset.**\n\n"
                        f"📊 **Nuevo estado | New status:**\n"
                        f"• Invitaciones de {self.target_user.display_name}: 0 | {self.target_user.display_name}'s invites: 0\n"
                        f"• Eventos de join asociados: 0 | Associated join events: 0"
                    ),
                    color=discord.Color.green()
                )
                success_embed.set_footer(text="HMFB X")
                
                print(f"🔄 Invitaciones de {self.target_user.name} reseteadas por {interaction.user.name}")
                ...
                print(f"🔄 Todas las invitaciones reseteadas por {interaction.user.name}")
    ...
    await interaction.response.send_message(
        embed=confirm_embed, 
        view=ResetConfirmView(target_user=usuario), 
        ephemeral=True
    )

# NUEVO COMANDO: Sistema de Embeds Avanzado
@bot.tree.command(name="embed", description="Crea embeds personalizados (Admin)")
@app_commands.checks.has_permissions(administrator=True)
@app_commands.describe(
    titulo="Título del embed",
    descripcion="Descripción del embed",
    color="Color en hexadecimal (ej: #FF0000) o nombre (red, blue, green)",
    canal="Canal donde enviar el embed (opcional)",
    thumbnail="URL de la imagen miniatura (opcional)",
    imagen="URL de la imagen principal (opcional)",
    footer="Texto del footer (opcional)",
    autor="Nombre del autor (opcional)",
    campo1_nombre="Nombre del campo 1 (opcional)",
    campo1_valor="Valor del campo 1 (opcional)",
    campo1_inline="¿Campo 1 en línea? (opcional)",
    campo2_nombre="Nombre del campo 2 (opcional)",
    campo2_valor="Valor del campo 2 (opcional)",
    campo2_inline="¿Campo 2 en línea? (opcional)",
    campo3_nombre="Nombre del campo 3 (opcional)",
    campo3_valor="Valor del campo 3 (opcional)",
    campo3_inline="¿Campo 3 en línea? (opcional)"
)
async def embed_command(
    ...
    footer: str = None,
    autor: str = None,
    campo1_nombre: str = None,
    campo1_valor: str = None,
    campo1_inline: bool = False,
    campo2_nombre: str = None,
    campo2_valor: str = None,
    campo2_inline: bool = False,
    campo3_nombre: str = None,
    campo3_valor: str = None,
    campo3_inline: bool = False
):
    """Crea embeds personalizados con múltiples opciones"""
    ...
    try:
        ...
        # Añadir imagen
        if imagen and is_valid_url(imagen):
            embed.set_image(url=imagen)
        
        # Añadir campos
        campos = [
            (campo1_nombre, campo1_valor, campo1_inline),
            (campo2_nombre, campo2_valor, campo2_inline),
            (campo3_nombre, campo3_valor, campo3_inline)
        ]
        ...
        # Confirmación
        success_embed = discord.Embed(
            title="✅ Embed Creado Exitosamente",
            description=f"El embed ha sido enviado a {target_channel.mention}",
            color=discord.Color.green()
        )
        success_embed.add_field(name="📝 Título", value=titulo, inline=True)
        success_embed.add_field(name="📊 Canal", value=target_channel.mention, inline=True)
        success_embed.add_field(name="🎨 Color", value=color, inline=True)
        
        if thumbnail:
            success_embed.add_field(name="🖼️ Thumbnail", value="✅ Sí", inline=True)
        if imagen:
            success_embed.add_field(name="🖼️ Imagen", value="✅ Sí", inline=True)
        
        field_count = sum(1 for nombre, valor, inline in campos if nombre and valor)
        success_embed.add_field(name="📋 Campos", value=field_count, inline=True)
        
        await interaction.response.send_message(embed=success_embed, ephemeral=True)
        
        print(f"📝 Embed creado por {interaction.user.name} en #{target_channel.name}")
        
    except Exception as e:
        error_embed = discord.Embed(
            title="❌ Error al Crear Embed",
